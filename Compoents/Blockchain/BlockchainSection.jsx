...
export default function BlockchainSection({ data }) {
  // Extract section data from API response
  const section = data?.blockchainIntroSection || {};
  ...
  const featuresList = features.items || defaultFeatures;
  const challengesList = challenge.items || defaultChallenges;
  const solutionsList = solution.items || defaultSolutions;

  return (
    <section className="relative bg-white py-24 overflow-hidden" aria-labelledby={section.ariaLabelledBy || "blockchain-section-heading"}>
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        {/* Main heading */}
        <h2 id={section.ariaLabelledBy || "blockchain-section-heading"} className="text-2xl sm:text-4xl font-bold text-center mb-6">
          <span className="bg-gradient-to-r from-blue-600 to-blue-800 bg-clip-text text-transparent">
            {heading.gradientText || "Building the Decentralized Future,"}
          </span>
          <span className="text-gray-900"> {heading.normalText || "One Block at a Time"}</span>
        </h2>

        {/* Introduction */}
        <p className="text-md md:text-lg text-gray-600 text-justify max-w-4xl mx-auto mb-16 leading-relaxed">
          {section.introduction || "Blockchain isn't just a buzzword anymore – it's the foundation of a new digital economy."}
        </p>
        ...
        <div className="grid lg:grid-cols-2 gap-8 mb-16">
          ...
          <article className="bg-gradient-to-br from-red-50 to-orange-50 rounded-3xl p-8 border border-red-100 shadow-lg">
            <div className="flex items-center gap-3 mb-6">
              ...
              <h3 className="text-2xl font-bold text-gray-900">{challenge.title || "The Challenge"}</h3>
            </div>
            <p className="text-gray-700 mb-6 leading-relaxed">
              {challenge.description || "Building robust, secure blockchain solutions requires deep technical expertise and a thorough understanding of decentralized systems."}
            </p>
          ...
          <article className="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-3xl p-8 border border-blue-200 shadow-lg">
            <div className="flex items-center gap-3 mb-6">
              ...
              <h3 className="text-2xl font-bold text-gray-900">{solution.title || "Our Solution"}</h3>
            </div>
            <p className="text-gray-700 mb-4 leading-relaxed">
              <span className="font-semibold text-blue-700">{solution.brand || "OpenSoftAI"}</span> {solution.intro || "isn't just another dev team – we've been in the blockchain space since Ethereum's early days."}
            </p>
            <p className="text-gray-700 mb-6 leading-relaxed">
              {solution.description || "Our team combines cutting-edge technical skills with real-world experience to deliver blockchain solutions that are innovative, secure, and scalable."}
            </p>
            <ul className="space-y-3">
        ...
        {/* Features grid */}
        <div className="mb-16">
          <h3 className="text-3xl font-bold text-center text-gray-900 mb-8">{features.heading || "What We Build"}</h3>
        ...
        <div className="text-center bg-gradient-to-r from-blue-600 to-blue-800 rounded-3xl p-12 shadow-2xl">
          <p className="text-lg md:text-xl text-white mb-8 font-medium">
            {cta.text || "From crypto wallets and exchange platforms to DeFi protocols and NFT marketplaces, we build blockchain solutions that power the next generation of digital businesses."}
          </p>
          <Link
            href={cta.button?.href || "/contact-us"}
          ...
          >
            {cta.button?.label || "Get Started Today"}
