...
export default function CryptoExpertiseSection({ data }) {
  // Extract section data
  const section = data?.cryptoExpertise || {};
  const header = section.header || {};
  const stats = section.stats || [];
  const expertiseAreasData = section.expertiseAreas || [];
  const platformsData = section.platforms || [];
  ...
  const expertiseAreas = expertiseAreasData.map(area => ({
    ...
    gradient: area.gradient || "from-blue-500 to-blue-600"
  }));
          ...
          <div className="space-y-6">
            <div className="bg-white rounded-3xl p-8 shadow-xl border border-blue-100">
              <div className="w-16 h-16 bg-gradient-to-br from-blue-500 to-indigo-600 rounded-2xl flex items-center justify-center mb-6 shadow-lg">
                <DynamicIcon name="Layers" className="w-8 h-8 text-white" />
              </div>
            ...
            <div className="bg-gradient-to-br from-blue-600 to-indigo-600 rounded-3xl p-8 shadow-xl text-white">
              <div className="w-16 h-16 bg-white/20 backdrop-blur-sm rounded-2xl flex items-center justify-center mb-6">
                <DynamicIcon name="CheckCircle" className="w-8 h-8 text-white" />
              </div>
          ...
          <div className="space-y-6">
            <div className="bg-white rounded-3xl p-8 shadow-xl border border-blue-100">
              <h3 className="text-2xl font-bold text-gray-900 mb-6 flex items-center gap-3">
                <DynamicIcon name="Zap" className="w-7 h-7 text-blue-600" />
                  ...
                  return (
                    ...
                    >
                      <div className={`w-12 h-12 bg-gradient-to-br ${area.gradient} rounded-xl flex items-center justify-center flex-shrink-0 shadow-md group-hover:scale-110 transition-transform`}>
                        <DynamicIcon name={area.icon} className="w-6 h-6 text-white" />
                      </div>
                      <div>
              ...
              return (
                ...
                >
                  <div className="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-2xl p-6 text-center hover:shadow-xl transition-all hover:-translate-y-1 border-2 border-blue-100 hover:border-blue-300">
                    <div className={`w-14 h-14 bg-gradient-to-br ${colorMap[platform.color] || colorMap.blue} rounded-xl flex items-center justify-center mx-auto mb-4 shadow-lg group-hover:scale-110 transition-transform`}>
                      <DynamicIcon name={platform.icon} className="w-7 h-7 text-white" />
                    </div>
        ...
        <div className="bg-gradient-to-r from-blue-600 via-indigo-600 to-blue-600 rounded-3xl p-12 shadow-2xl text-center relative overflow-hidden">
          ...
          <div className="relative z-10">
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              {cta.primary && (
                <Link href={cta.primary.link || "/contact-us"} className="bg-white text-blue-600 px-8 py-4 rounded-full font-bold text-lg shadow-xl hover:shadow-2xl hover:scale-105 transition-all">
                  {cta.primary.label}
                </Link>
              )}
              {cta.secondary && (
                <Link href={cta.secondary.link || "/portfolio"} className="bg-transparent border-2 border-white text-white px-8 py-4 rounded-full font-bold text-lg hover:bg-white/10 transition-all">
                  {cta.secondary.label}
                </Link>
              )}
            </div>
