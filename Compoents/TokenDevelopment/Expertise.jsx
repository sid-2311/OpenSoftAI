...
export default function ExpertiseSection({ data }) {
  ...
  const coreMessage = section.coreMessage || {};
  const expertiseCardsData = section.expertiseCards || [];
  ...
  const expertise = expertiseCardsData.map(card => ({
    title: card.title,
            ...
            return (
              ...
              >
                ...
                <div className="relative bg-slate-800 bg-opacity-50 backdrop-blur-xl border border-blue-500 border-opacity-30 rounded-2xl p-8 hover:border-opacity-60 transition-all duration-300 hover:-translate-y-2">
                  <DynamicIcon name={stat.icon} className="w-12 h-12 text-blue-400 mb-4" />
          ...
          <div className={`transition-all duration-1000 ${isVisible ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-10'}`}>
            <div className="bg-gradient-to-br from-slate-800 to-slate-900 rounded-3xl p-8 border border-blue-500 border-opacity-30 backdrop-blur-xl h-full">
              ...
              <div className="space-y-6">
                {coreMessage.points?.map((point, idx) => (
                  <div key={idx} className="flex items-start">
                    <div className="w-2 h-2 bg-blue-400 rounded-full mt-2 mr-4 animate-pulse" style={{ animationDelay: `${idx * 0.5}s` }}></div>
          ...
          <div className={`space-y-6 transition-all duration-1000 ${isVisible ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-10'}`}>
            {expertise.map((item, idx) => {
              ...
              return (
                <div
                  key={idx}
                  className={`relative group cursor-pointer transition-all duration-500 ${activeCard === idx ? 'scale-105' : 'scale-100'
                    }`}
                  onMouseEnter={() => setActiveCard(idx)}
                >
                  <div className={`absolute inset-0 bg-gradient-to-r from-blue-600 to-blue-400 rounded-2xl blur-lg transition-opacity duration-500 ${activeCard === idx ? 'opacity-40' : 'opacity-0'
                    }`}></div>
                  <div className={`relative bg-slate-800 bg-opacity-70 backdrop-blur-xl border rounded-2xl p-6 transition-all duration-500 ${activeCard === idx
                    ? 'border-blue-400 border-opacity-80'
                    : 'border-blue-500 border-opacity-20'
                    }`}>
                    <div className={`flex items-start transition-all duration-500 ${activeCard === idx ? 'text-blue-300' : 'text-blue-400'
                      }`}>
